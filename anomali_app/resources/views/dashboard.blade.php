...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .title-block p{color:var(--muted);font-size:12.5px;margin-top:4px;max-width:560px;}
  .live-badge{
  ...
  .clock span{color:var(--text);font-weight:600;font-size:13px;}
  .hint{
    font-size:11px;color:#5b6a8a;margin-bottom:16px;display:flex;align-items:center;gap:6px;
  }

  .flow-panel{
    background:var(--panel);border:1px solid var(--border);
    border-radius:14px;padding:20px 18px 26px;margin-bottom:22px;
  ...
  .flow-step{
    ...
    border-radius:12px;padding:14px 16px;min-width:168px;flex:1;
    display:flex;flex-direction:column;gap:6px;position:relative;
    cursor:pointer;transition:transform .15s, border-color .15s;
  }
  .flow-step:hover{transform:translateY(-3px);border-color:#3b82f6;}
  .flow-step .icon{
    width:34px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;
    font-size:16px;margin-bottom:4px;
  }
  .flow-step h4{font-size:13px;font-weight:700;}
  .flow-step p{font-size:11px;color:var(--muted);line-height:1.4;}
  .flow-step .tap{font-size:9.5px;color:#3b82f6;margin-top:2px;font-weight:600;}
  .arrow{
    display:flex;align-items:center;justify-content:center;color:#33456e;font-size:20px;
    padding:0 6px;flex-shrink:0;
  }
  .icon-blue{background:rgba(59,130,246,.15);color:var(--accent);}
  .icon-purple{background:rgba(139,92,246,.15);color:var(--purple);}
  ...
  .chart-title h3{font-size:13.5px;font-weight:700;}
  ...
  .alert{
    ...
    border-radius:8px;padding:10px 12px;cursor:pointer;transition:transform .15s, border-color .15s;
  }
  .alert:hover{transform:translateX(3px);border-color:#3b82f6;}
  .alert.warn{border-left-color:var(--amber);}
  .alert.ok{border-left-color:var(--green);}
  .alert .a-icon{font-size:15px;margin-top:1px;}
  .alert .a-body b{font-size:12.5px;}
  .alert .a-body p{font-size:11px;color:var(--muted);margin-top:2px;}
  .alert .a-time{font-size:10px;color:#5b6a8a;white-space:nowrap;margin-left:auto;}

  .status-row{display:flex;gap:18px;margin-top:4px;flex-wrap:wrap;}
  .status-item{display:flex;align-items:center;gap:8px;font-size:11.5px;color:var(--muted);}
  .status-item .sd{width:8px;height:8px;border-radius:50%;}

  table.roles{width:100%;border-collapse:collapse;font-size:12px;}
  table.roles th{
    text-align:left;color:var(--muted);font-weight:600;font-size:10.5px;
    text-transform:uppercase;letter-spacing:.4px;padding-bottom:8px;border-bottom:1px solid var(--border);
  }
  table.roles td{padding:9px 4px;border-bottom:1px solid var(--border);}
  table.roles tr.rowclick{cursor:pointer;}
  table.roles tr.rowclick:hover td{color:#fff;}
  ...
  footer{text-align:center;color:#4b5877;font-size:10.5px;margin-top:10px;}

  /* ===== MODAL ===== */
  .overlay{
    position:fixed;inset:0;background:rgba(3,7,15,0.72);backdrop-filter:blur(2px);
  ...
  .modal{
    ...
    padding:22px;position:relative;animation:pop .18s ease-out;
  }
  ...
  .mrow{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:12.5px;}
  .mrow span:first-child{color:var(--muted);}
  .mrow span:last-child{font-weight:600;}
  .mbar-wrap{margin-top:14px;}
  .mbar{height:8px;border-radius:4px;background:#1f2c47;overflow:hidden;margin-top:5px;}
  .mbar i{display:block;height:100%;border-radius:4px;}
  .mfoot{margin-top:16px;font-size:11.5px;color:var(--muted);line-height:1.5;background:var(--panel2);border:1px solid var(--border);border-radius:10px;padding:10px 12px;}
  .modal .status-pill{display:inline-block;font-size:10.5px;font-weight:700;padding:3px 10px;border-radius:20px;margin-bottom:2px;}
  ...
  @media (max-width: 860px){
    ...
    .title-block p{font-size:12px;max-width:100%;}
    .flow-row{flex-direction:column;overflow-x:visible;}
    .flow-step{min-width:100%;}
    .arrow{transform:rotate(90deg);padding:2px 0;justify-content:center;}
    ...
    .chart-title{flex-direction:column;align-items:flex-start;gap:6px;}
    table.roles{font-size:11px;}
    table.roles th:nth-child(3), table.roles td:nth-child(3){display:none;}
    .alert-list{max-height:none;}
  }
  @media (max-width: 480px){
    .kpi-grid{grid-template-columns:1fr 1fr;}
    .kpi-grid .kpi:last-child{grid-column:span 2;}
    .title-block h1{font-size:16px;}
    .live-badge{font-size:10px;padding:4px 8px;}
    svg{height:150px !important;}
    .modal{padding:18px;}
  }
</style>
</head>
...
<div class="header">
  <div class="title-block">
    <h1>🧭 ANOMALI — Sistem Deteksi Dini Cost &amp; Norma Berbasis AI <span class="live-badge"><span class="dot"></span>LIVE MONITORING</span></h1>
    <p>Pemantauan realisasi cost dan norma kerja secara real-time, deteksi anomali otomatis oleh AI, serta klarifikasi penyebab over budget untuk mendukung pengambilan keputusan Kepala Kebun &amp; Asisten Afdeling.</p>
  </div>
  <div class="clock">Update terakhir<br><span>Hari ini, 14:32 WIB</span></div>
</div>

<div class="hint">👉 Semua kartu, langkah alur, dan alert di bawah ini bisa diklik untuk melihat detail data (dummy).</div>

<!-- ARSITEKTUR ALUR SISTEM -->
<div class="flow-panel">
  <div class="panel-label">Arsitektur Alur Sistem</div>
  <div class="flow-row">
    <div class="flow-step" onclick="openFlow('input')">
      <div class="icon icon-blue">📥</div>
      <h4>Data Input</h4>
      <p>Realisasi cost, norma kerja, budget &amp; standar per afdeling.</p>
      <div class="tap">Lihat detail ›</div>
    </div>
    <div class="arrow">➜</div>
    <div class="flow-step" onclick="openFlow('ai')">
      <div class="icon icon-purple">🤖</div>
      <h4>AI Analysis Engine</h4>
      <p>Membandingkan realisasi vs budget/standar secara kontinu.</p>
      <div class="tap">Lihat detail ›</div>
    </div>
    <div class="arrow">➜</div>
    <div class="flow-step" onclick="openFlow('detect')">
      <div class="icon icon-amber">⚠️</div>
      <h4>Deteksi Anomali</h4>
      <p>Menandai cost/norma yang berpotensi over budget lebih awal.</p>
      <div class="tap">Lihat detail ›</div>
    </div>
    <div class="arrow">➜</div>
    <div class="flow-step" onclick="openFlow('notif')">
      <div class="icon icon-red">🔔</div>
      <h4>Notifikasi &amp; Alert</h4>
      <p>Peringatan otomatis ke Kepala Kebun &amp; Asisten Afdeling.</p>
      <div class="tap">Lihat detail ›</div>
    </div>
    <div class="arrow">➜</div>
    <div class="flow-step" onclick="openFlow('action')">
      <div class="icon icon-green">✅</div>
      <h4>Klarifikasi &amp; Tindakan</h4>
      <p>Klarifikasi penyebab, tindakan korektif berbasis data.</p>
      <div class="tap">Lihat detail ›</div>
    </div>
  </div>
</div>
...
<div class="grid">
  <div class="col">
    ...
    <div class="card">
      ...
      <div class="kpi-grid">
        <div class="kpi" onclick="openKpi('afdeling')">
          <div class="label">Afdeling Terpantau</div>
          <div class="value">128</div>
          <div class="delta down">▲ aktif termonitor</div>
        </div>
        <div class="kpi" onclick="openKpi('anomali')">
          <div class="label">Anomali Terdeteksi</div>
          <div class="value" style="color:var(--amber)">14</div>
          <div class="delta up">▲ 3 sejak kemarin</div>
        </div>
        <div class="kpi" onclick="openKpi('klarifikasi')">
          <div class="label">Sudah Diklarifikasi</div>
          <div class="value" style="color:var(--green)">9</div>
          <div class="delta down">▼ tindak lanjut tepat waktu</div>
        </div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <svg viewBox="0 0 560 190" width="100%" height="190">
        ...
        <polyline fill="none" stroke="#f59e0b" stroke-width="2" stroke-dasharray="5,4"
          points="0,90 40,88 80,86 120,90 160,88 200,86 240,88 280,86 320,88 360,86 400,88 440,86 480,88 520,86 560,88"/>
        <polygon fill="rgba(59,130,246,0.15)" points="0,130 40,120 80,100 120,95 160,70 200,80 240,60 280,45 320,55 360,40 400,50 440,35 480,55 520,42 560,30 560,190 0,190"/>
        <polyline fill="none" stroke="#3b82f6" stroke-width="2.5"
          points="0,130 40,120 80,100 120,95 160,70 200,80 240,60 280,45 320,55 360,40 400,50 440,35 480,55 520,42 560,30"/>
        <circle class="pt" cx="240" cy="60" r="6" fill="#ef4444" onclick="openPoint('Hari ke-13','Afdeling III','Biaya Pupuk',118)"/>
        <circle class="pt" cx="360" cy="40" r="6" fill="#ef4444" onclick="openPoint('Hari ke-19','Afdeling V','Biaya Pemeliharaan',124)"/>
        <circle class="pt" cx="480" cy="55" r="6" fill="#ef4444" onclick="openPoint('Hari ke-25','Afdeling IX','Norma Semprot',109)"/>
        <circle class="pt" cx="560" cy="30" r="7" fill="#3b82f6" onclick="openPoint('Hari ini','Seluruh Afdeling','Realisasi Terkini',97)">
          <animate attributeName="r" values="6;9;6" dur="1.4s" repeatCount="indefinite"/>
        </circle>
      </svg>
      <div class="status-row">
        <div class="status-item"><span class="sd" style="background:#ef4444"></span>Klik titik merah untuk lihat detail anomali</div>
        <div class="status-item"><span class="sd" style="background:#3b82f6"></span>Data terkini diperbarui otomatis</div>
      </div>
    </div>
    ...
    <div class="card">
      ...
      <table class="roles">
        <tr><th>Peran</th><th>Fungsi Utama</th><th>Akses</th></tr>
        <tr class="rowclick" onclick="openRole('asisten')"><td>Asisten Afdeling</td><td>Menerima alert, memberi klarifikasi awal</td><td><span class="tag tag-blue">Input &amp; Respon</span></td></tr>
        <tr class="rowclick" onclick="openRole('kakebun')"><td>Kepala Kebun</td><td>Memantau afdeling, menyetujui tindakan</td><td><span class="tag tag-amber">Monitoring</span></td></tr>
        <tr class="rowclick" onclick="openRole('manajemen')"><td>Manajemen</td><td>Melihat rekap indikator keberhasilan</td><td><span class="tag tag-green">Dashboard</span></td></tr>
      </table>
    </div>
  </div>
  ...
  <div class="col">
    ...
    <div class="card" style="flex:1;">
      <div class="panel-label">Status Komponen Sistem</div>
      <div style="display:flex;flex-direction:column;gap:10px;">
        <div class="status-item"><span class="sd" style="background:#22c55e"></span>Koneksi Data Sumber (ERP/Operasional) — Normal</div>
        <div class="status-item"><span class="sd" style="background:#22c55e"></span>AI Analysis Engine — Berjalan, latensi rendah</div>
        <div class="status-item"><span class="sd" style="background:#22c55e"></span>Modul Notifikasi (Alert ke User) — Aktif</div>
        <div class="status-item"><span class="sd" style="background:#f59e0b"></span>Sinkronisasi Data Afdeling IX — Sedikit tertunda</div>
      </div>
    </div>
  </div>
</div>

<footer>Gambaran sistem ANOMALI — ilustrasi konsep monitoring real-time untuk keperluan presentasi. Seluruh data bersifat dummy.</footer>
...
const kpiData = {
  afdeling:{title:"🌴 Afdeling Termonitor", sub:"Cakupan pemantauan sistem saat ini", rows:[["Total afdeling","128"],["Status koneksi normal","124 afdeling"],["Sinkronisasi tertunda","4 afdeling"],["Wilayah","3 kebun inti"]], note:"Seluruh afdeling terhubung ke sistem monitoring cost & norma secara real-time."},
  anomali:{title:"⚠️ Anomali Terdeteksi", sub:"Ringkasan kasus anomali aktif", rows:[["Total anomali aktif","14"],["Baru (24 jam terakhir)","3"],["Kategori cost","9 kasus"],["Kategori norma","5 kasus"]], note:"Jumlah anomali dihitung dari kasus yang belum ditutup / belum sepenuhnya diklarifikasi."},
  klarifikasi:{title:"✅ Sudah Diklarifikasi", sub:"Progress penyelesaian klarifikasi", rows:[["Total diklarifikasi","9 dari 14"],["Rata-rata waktu klarifikasi","4.5 jam"],["Tindakan korektif dijalankan","6 kasus"],["Menunggu tindak lanjut","5 kasus"]], note:"Target internal: klarifikasi selesai dalam waktu kurang dari 8 jam sejak notifikasi terkirim."}
};
...
function openAlert(id){
  ...
  const d = a.detail;
  const barColor = d.persen > 105 ? '#ef4444' : (d.persen > 100 ? '#f59e0b' : '#22c55e');
  showModal(`
    <button class="close" onclick="closeModal()">✕</button>
    <span class="status-pill" style="background:${d.statusColor}22;color:${d.statusColor}">${d.status}</span>
    <h2>${a.icon} ${a.title}</h2>
    <div class="sub">${a.time} · PIC: ${d.pic}</div>
    <div class="mrow"><span>Budget</span><span>${d.budget}</span></div>
    <div class="mrow"><span>Realisasi</span><span>${d.realisasi}</span></div>
    <div class="mbar-wrap">
      <div class="mrow" style="border-bottom:none;"><span>Persentase realisasi</span><span>${d.persen}%</span></div>
      <div class="mbar"><i style="width:${Math.min(d.persen,140)/1.4}%;background:${barColor}"></i></div>
    </div>
    <div class="mfoot"><b>Catatan:</b> ${d.catatan}</div>
  `);
}
